...
No test suite. Gates are `typecheck` + `build` + launching the built app. Main-process code (`out/main`) is loaded at startup only — F5 reloads the renderer, not the launcher/parsers.
...
- `hosts.ts` — the only writer. `saveHost` renders exactly `sshm create`'s five-line block; `deleteHost`/`deleteKey` refuse anything outside `~/.ssh/hosts/` / `~/.ssh/keys/`; `ensureInclude` prepends the Include line; `testHost` = BatchMode ssh with 5 s timeout; keys via `ssh-keygen`.
