...
function keepOpenScript(argv: string[]): string {
  ...
  // `trap : INT`: Ctrl-C at a password prompt must kill ssh, not this wrapper — otherwise
  // the terminal reports "Program 'sh' crashed" instead of dropping into a shell.
  return `trap : INT; ${ssh}; s="\${SHELL:-sh}"; case "$s" in /*) ;; */*) s=sh ;; esac; exec "$s"`
}
...
/**
 * `ssh <alias>` — the user's config supplies key / port / user. Values the user typed
 * deliberately (userSpecified) are kept as explicit options; app defaults are not.
 */
function aliasArgv(alias: string, options: TerminalLaunchOptions): string[] {
  const argv = ['ssh']
  if (options.userSpecified) {
    if (options.username) argv.push('-l', options.username)
...
export async function launchNativeTerminal(options: TerminalLaunchOptions): Promise<TerminalLaunchResult> {
  ...
  const command = formatArgv(argv, flavour)
  ...
  try {
    if (process.platform === 'win32') {
      ...
      try {
        ...
        return { success: true, terminal: 'Windows Terminal', command, alias: alias ?? undefined }
      } catch {
        ...
        return { success: true, terminal: basename(powershell, '.exe'), command, alias: alias ?? undefined }
      }
    }
    ...
    if (process.platform === 'darwin') {
      ...
      return { success: true, terminal: 'Terminal.app', command, alias: alias ?? undefined }
    }
    ...
    return { success: true, terminal: term.name, command, alias: alias ?? undefined }
  } catch (err: any) {
