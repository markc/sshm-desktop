...
/** Render an argv as a command line for the target shell — every word quoted unless provably safe. */
export function formatArgv(argv: string[], flavour: ShellFlavour): string {
  return argv
    .map((w) => {
...
/** Human-readable ssh command line for error messages / clipboard, quoted for the target shell. */
export function formatSshCommand(options: TerminalLaunchOptions, flavour: ShellFlavour): string {
  return formatArgv(sshArgv(options), flavour)
}
